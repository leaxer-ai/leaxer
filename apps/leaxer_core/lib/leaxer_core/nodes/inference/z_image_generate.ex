defmodule LeaxerCore.Nodes.Inference.ZImageGenerate do
  @moduledoc """
  Generate images using Z-Image-Turbo via stable-diffusion.cpp.

  Z-Image models require:
  - Separate diffusion model (Z-Image-Turbo)
  - FLUX schnell VAE (--vae)
  - Qwen3 4B LLM encoder (--llm)
  - Very low VRAM requirements (~4GB)

  Returns base64 output.

  Perfect for fast generation on limited hardware.
  """

  use LeaxerCore.Nodes.Behaviour

  @impl true
  def type, do: "ZImageGenerate"

  @impl true
  def label, do: "Z-Image Generate"

  @impl true
  def category, do: "Inference/Generate"

  @impl true
  def description, do: "Generate images using Z-Image-Turbo (low VRAM)"
  ...
  def input_spec do
    %{
      diffusion_model: %{
        ...
        default: "",
        description: "Path to Z-Image-Turbo diffusion model"
      },
      vae: %{
        type: :string,
        label: "VAE",
      ...
      llm: %{
        ...
        label: "LLM",
        default: "",
        description: "Path to Qwen3 4B LLM encoder (GGUF)"
      },
      ...
      steps: %{type: :integer, label: "STEPS", default: 4, min: 1, max: 50, step: 1},
      cfg_scale: %{type: :float, label: "CFG SCALE", default: 3.0, min: 1.0, max: 15.0, step: 0.5},
  ...
  @impl true
  def ui_component, do: {:custom, "ZImageGenerateNode"}
  ...
  def process(inputs, config) do
    ...
    cond do
      is_nil(diffusion_model) or diffusion_model == "" ->
        {:error, "Diffusion model is required (Z-Image-Turbo)"}

      is_nil(vae) or vae == "" ->
        {:error, "VAE model is required (use FLUX schnell VAE)"}

      is_nil(llm) or llm == "" ->
        {:error, "LLM encoder is required (Qwen3 4B GGUF)"}
      ...
      true ->
        opts = [
          ...
          steps: inputs["steps"] || config["steps"] || 4,
          cfg_scale: inputs["cfg_scale"] || config["cfg_scale"] || 3.0,
