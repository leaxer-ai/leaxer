defmodule LeaxerCore.Nodes.Inference.OvisImageGenerate do
  @moduledoc """
  Generate images using Ovis 2.5 via stable-diffusion.cpp.

  Ovis models require:
  - Separate diffusion model (--diffusion-model)
  - FLUX schnell VAE (--vae)
  - Ovis 2.5 LLM encoder (--llm)
  - Recommended CFG scale of 5.0
  ...
  @impl true
  def type, do: "OvisImageGenerate"

  @impl true
  def label, do: "Ovis Image Generate"

  @impl true
  def category, do: "Inference/Generate"

  @impl true
  def description, do: "Generate images using Ovis 2.5 LLM"
  ...
  def input_spec do
    %{
      diffusion_model: %{
        ...
        default: "",
        description: "Path to Ovis diffusion model"
      },
      vae: %{
        type: :string,
        label: "VAE",
      ...
      llm: %{
        ...
        label: "LLM",
        default: "",
        description: "Path to Ovis 2.5 LLM encoder (GGUF)"
      },
  ...
  @impl true
  def ui_component, do: {:custom, "OvisImageGenerateNode"}
  ...
  def process(inputs, config) do
    ...
    cond do
      ...
      is_nil(llm) or llm == "" ->
        {:error, "LLM encoder is required (Ovis 2.5 GGUF)"}
