...
export const QwenImageEditNode = memo(({ id, data, selected }: NodeProps) => {
  ...
  const zeroCondConnected = useIsHandleConnected(id, 'zero_cond_t', 'target');

  const variant = (data.variant as string) || '2509';
  ...
  return (
    <BaseNode
      ...
      selected={selected}
      handles={[
        { id: 'source_image', type: 'target', position: Position.Left, label: 'SOURCE IMAGE', dataType: 'IMAGE' },
        ...
        { id: 'llm_vision', type: 'target', position: Position.Left, label: 'LLM VISION', dataType: 'STRING' },
        { id: 'edit_prompt', type: 'target', position: Position.Left, label: 'EDIT PROMPT', dataType: 'STRING' },
        { id: 'image', type: 'source', position: Position.Right, label: 'IMAGE', dataType: 'IMAGE' },
      ]}
    >
      <div className="w-full space-y-3">
        {/* Info */}
        <div className="flex items-start gap-2 p-2 bg-purple-500/10 border border-purple-500/30 rounded text-xs text-purple-200">
          <Wand2 className="w-4 h-4 flex-shrink-0 mt-0.5" />
          <span>Edit images using Qwen-Image 2509/2511. Connect a source image.</span>
        </div>
        ...
        <div className="w-full space-y-1.5">
          ...
          <select
            value={variant}
            onChange={(e) => updateNodeData(id, { variant: e.target.value })}
            className="nodrag nowheel w-full h-8 text-xs bg-surface-1/50 border border-overlay-0 rounded-md px-2 text-text-primary"
          >
            <option value="2509">Qwen-Image 2509 (Vision)</option>
            <option value="2511">Qwen-Image 2511 (Zero-Cond)</option>
          </select>
        </div>
        ...
        <div className="w-full space-y-1.5">
          ...
          <Input
            ...
            placeholder="Path to diffusion model"
        ...
        <div className="w-full space-y-1.5">
          ...
          <Input
            ...
            onKeyDown={(e) => e.stopPropagation()}
            placeholder="Path to LLM GGUF"
            className="nodrag nowheel w-full h-8 text-xs bg-surface-1/50 border-overlay-0"
          />
        </div>

        {/* LLM Vision (for 2509 variant) */}
        {variant === '2509' && (
          <div className="w-full space-y-1.5">
            <Label className="text-xs text-text-muted">LLM VISION PATH (2509)</Label>
            <Input
              type="text"
              value={(data.llm_vision as string) || ''}
              onChange={(e) => updateNodeData(id, { llm_vision: e.target.value })}
              onKeyDown={(e) => e.stopPropagation()}
              placeholder="Path to vision LLM GGUF"
              className="nodrag nowheel w-full h-8 text-xs bg-surface-1/50 border-overlay-0"
            />
          </div>
        )}
        ...
        {/* Zero Cond T (for 2511 variant) */}
        {variant === '2511' && (
          <div className="relative w-full space-y-2">
            <ParameterHandle id="zero_cond_t" dataType="FLOAT" />
            <div className="flex justify-between">
              <Label className="text-xs text-text-muted">ZERO COND T</Label>
              <span className="text-xs text-text-secondary tabular-nums">
                {zeroCondConnected ? '–' : String(data.zero_cond_t ?? 0.5)}
              </span>
            </div>
            <Slider
              value={(data.zero_cond_t as number) ?? 0.5}
              onChange={(v: number) => updateNodeData(id, { zero_cond_t: v })}
              min={0}
              max={1}
              step={0.05}
              disabled={zeroCondConnected}
              className={cn('nodrag nowheel w-full', zeroCondConnected && 'opacity-50')}
            />
            <p className="text-[10px] text-text-muted">
              Zero conditioning threshold for 2511 variant
            </p>
          </div>
        )}
...
QwenImageEditNode.displayName = 'QwenImageEditNode';
